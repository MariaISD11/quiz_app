  ...
  return (
    <Layout>
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl font-bold tracking-tight">{t(LOCALE_KEYS.CATEGORIES)}</h2>
        <Button onClick={() => setIsAdding(true)}>
          <Plus size={20} />
          {t(LOCALE_KEYS.ADD_CATEGORY)}
        </Button>
      </div>

      {isAdding && (
        <Card className="p-6 mb-6 animate-in slide-in-from-top-4 duration-300">
          <div className="space-y-4">
            <input
              autoFocus
            ...
            <div className="flex gap-2">
              <Button onClick={handleAdd}>{t(LOCALE_KEYS.SAVE)}</Button>
              <Button variant="secondary" onClick={() => setIsAdding(false)}>{t(LOCALE_KEYS.CANCEL)}</Button>
            </div>
          </div>
        </Card>
      )}
          ...
          <Card key={cat.id} className="group p-6 cursor-pointer relative" onClick={() => onSelectCategory(cat)}>
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-gray-500 text-sm font-medium">{getCategoryStats(cat.id)} {t(LOCALE_KEYS.CARDS)}</p>
              </div>
